...
CREATE OR REFRESH STREAMING LIVE TABLE recordings_enriched
  (CONSTRAINT valid_hearrate EXPECT (heartrate > 0) ON VIOLATION DROP ROW)
AS SELECT 
  ...
  INNER JOIN STREAM(live.pii) b on a.mrn = b.mrn;
...
--       FROM STREAM(live.recordings_enriched) GROUP BY mrn, name, Date(time);
